...
def make_muzzle_flash_frames(steps=10):
    ...
    spread_start = random.uniform(100, 140)
    spread_end = random.uniform(40, 70)

    falloff_scale = random.uniform(2.5, 4.0)
    intensity_scale = random.uniform(0.5, 0.8)

    for i in range(steps):
        t = i / (steps - 1)

        f = 1.0 + t * falloff_scale

        s = (1.0 - t) * intensity_scale * random.uniform(0.9, 1.1)

        c = tuple(min(255, int(ch * s)) for ch in base_color)

        spread = spread_start - t * (spread_start - spread_end)
